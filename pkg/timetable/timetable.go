...
func init() {
	data, err := os.ReadFile("storage/jsons/dict.json")
	if err != nil {
	...
	dataTeachers, err := os.ReadFile("storage/jsons/teachers.json")
	if err != nil {
	...
	dataWeeks, err := os.ReadFile("storage/jsons/weeks.json")
	if err != nil {
	...
	lexicon, err := os.ReadFile("storage/jsons/lexicon.json")
	if err != nil {
		log.Fatal(err)
	}
	...
	if err := json.Unmarshal(lexicon, &values); err != nil {
		log.Fatal(err)
	}
...
func FormatingDate(role string) (string, string, error) {
	var day, week string
	var err error
